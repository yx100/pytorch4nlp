...
class CNNEncoder(nn.Module):
    def __init__(self,
        ...
        self.conv_layer = nn.Conv2d(in_channels=1,
                                    ...
                                    padding=(window_size - 1, 0) if padding else 0,
                                    bias=bias)
    ...
    def forward(self, inputs, lengths=None):
        ...
        """
        conv_result = self.forward_conv(inputs)
        if lengths is not None:
            if self.padding:
                lengths = lengths + (self.window_size - 1)
            else:
                lengths = lengths - (self.window_size + 1)
        dp_result = self.dropout_layer(conv_result)
        return get_pooling(dp_result, pooling_type=self.pooling_type, lengths=lengths)
