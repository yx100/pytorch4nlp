...
def test_split():
    ...
    zero = Variable(zero)
    lens = Variable(lens)
    left = relative_postitionm2mask(zero, left_position, torch.max(lens).data[0])
    middle = relative_postitionm2mask(left_position, right_position, torch.max(lens).data[0])
    right = relative_postitionm2mask(right_position, lens, torch.max(lens).data[0])
    print(left)
    print(middle)
    print(right)
